...
const globalStyles = (isLow) => `
  ...
  body { font-family: 'Manrope', sans-serif; background: #020305; margin: 0; overflow-x: hidden; color: #f8fafc; -webkit-font-smoothing: antialiased; }
  ::-webkit-scrollbar { width: 8px; }
  ::-webkit-scrollbar-track { background: #0b0c15; }
  ::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.2); border-radius: 4px; }
  ::-webkit-scrollbar-thumb:hover { background: #F43F5E; }

  .space-bg { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: radial-gradient(circle at 50% 120%, #1e1b4b 0%, #000000 70%); z-index: -10; pointer-events: none; }
  .stars-lg, .stars-sm { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-repeat: repeat; animation: ${isLow ? 'none' : 'drift'} 100s linear infinite; }
  .stars-lg { background-image: radial-gradient(2.5px 2.5px at 50px 50px, #fff, transparent), radial-gradient(2px 2px at 150px 250px, #fff, transparent), radial-gradient(2.5px 2.5px at 300px 100px, rgba(255,255,255,0.9), transparent); background-size: 500px 500px; opacity: 0.9; }
  .stars-sm { background-image: radial-gradient(1px 1px at 20px 30px, rgba(255,255,255,0.7), transparent), radial-gradient(1px 1px at 90px 80px, rgba(255,255,255,0.7), transparent); background-size: 500px 500px; opacity: 0.5; animation-duration: 150s; }
  @keyframes drift { from { background-position: 0 0; } to { background-position: 500px 500px; } }

  .horizontal-scroll-container { display: flex; gap: 20px; overflow-x: auto; padding: 10px 0 20px 0; scrollbar-width: none; }
  .horizontal-scroll-container::-webkit-scrollbar { display: none; }

  .load-more-btn { display: block; margin: 50px auto; padding: 14px 50px; background: transparent; color: #fff; border: 1px solid rgba(255,255,255,0.2); border-radius: 12px; font-weight: 700; cursor: pointer; transition: all 0.3s ease; }
  .load-more-btn:hover:not(:disabled) { background: #fff; color: #000; border-color: #fff; transform: translateY(-2px); box-shadow: 0 10px 20px rgba(255,255,255,0.1); }
...
const Home = ({ onCardClick, lowGraphics, toggleGraphics }) => {
  ...
  // Запрос для популярных (с пагинацией)
  const {
    data: popularData,
    fetchNextPage,
    hasNextPage,
    ...
    queryKey: ['popular', sortType],
    queryFn: async ({ pageParam = 1 }) => {
      const res = await fetch(`${apiUrl}/popular?sort=${sortType}&page=${pageParam}`);
      return res.json();
    },
    getNextPageParam: (lastPage, allPages) => {
      // Бэкенд возвращает по 30 элементов. Если пришло меньше, значит конец.
      return lastPage.length === 30 ? allPages.length + 1 : undefined;
    },
    enabled: !isSearchActive // Не делаем запрос, если открыт поиск
  });

  // Запрос для поиска (без пагинации, так как бэкенд возвращает сразу всё)
  const {
    data: searchData,
    ...
    enabled: false // Делаем запрос только по явному клику
  });
  ...
  const isLoading = isSearchActive ? (isSearchLoading || isSearchFetching) : isPopularLoading;
  // Собираем все страницы из пагинации в один плоский массив
  const popularList = popularData?.pages.flat() || [];
  const displayedList = isSearchActive ? (searchData || []) : popularList;
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 25px', zIndex: 1, position: 'relative' }}>
        ...
        <section style={{ marginTop: '30px', position: 'relative', zIndex: 2, paddingBottom: '60px' }}>
          {isLoading ? (
            ...
            <>
              {favorites.length > 0 && !isSearchActive && (
              ...
              {displayedList.length === 0 && isSearchActive && !isLoading && (
                <div style={{ textAlign: 'center', padding: '50px', color: '#94a3b8' }}>По вашему запросу ничего не найдено :(</div>
              )}
